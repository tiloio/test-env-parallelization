...
import { assertEquals, assertStringIncludes } from "https://deno.land/std@0.119.0/testing/asserts.ts";
import { sleep } from "./sleep.ts";
import { cleanTemp, createOrReturnTempFile } from "./create-or-return-temp-file.ts";

Deno.test('create or return temp file should wait until creation is finsihed and return file content', async () => {
...
Deno.test('create or return temp file should fail if lock file is not release after specified time', async () => {
    ...
    const runLater = async () => {
        await sleep(1);
        return createOrReturnTempFile(secondResource, { waitTimeout: 1, maxWaitLoops: 0 });
    };
